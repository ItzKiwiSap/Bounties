...
public class YourBountiesInventory {
	...
	public static void openAllBountiesInventory(Bounties plugin, Player p, int page) {
		...
		for(Inventory inv : inventories) {
			if(inventories.size() > 1) {
			    ...
			    previousMeta.setDisplayName(Utils.tr(plugin.getResources().getMessages().getString("Messages.Inventories.YourBounties.Previous.DisplayName")));
		    ...
		    closeMeta.setDisplayName(Utils.tr(plugin.getResources().getMessages().getString("Messages.Inventories.YourBounties.Close.DisplayName")));
		    ...
		    if(inventories.get(inventories.size() - 1) != inv && inventories.get(0) != inv) {
			    ...
			    nextMeta.setDisplayName(Utils.tr(plugin.getResources().getMessages().getString("Messages.Inventories.YourBounties.Next.DisplayName")));
		    ...
		    for(int i = 9; i < 45; i++) {
		    	if(plugin.getManager().getBounties() != null) {
				    for(Bounty bounty : plugin.getManager().getBounties()) {
				    	if(!bounty.isInInventory() && bounty.getByPlayer().getUniqueId().toString().equals(p.getUniqueId().toString())) {
					    	headslot++;
